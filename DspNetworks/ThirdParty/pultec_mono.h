...
#include "src/pultec_mono.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct pultec_monoMetaData {
		SN_NODE_ID("pultec_mono");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using pultec_mono = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _pultec_mono, pultec_monoMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
