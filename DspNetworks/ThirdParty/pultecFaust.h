...
#include "src/pultecFaust.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct pultecFaustMetaData {
		SN_NODE_ID("pultecFaust");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using pultecFaust = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _pultecFaust, pultecFaustMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
